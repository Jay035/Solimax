...
import { mainnet, optimism, fantomTestnet, optimismGoerli, fantom } from 'wagmi/chains';
import { publicProvider } from 'wagmi/providers/public'
 

const { provider } = configureChains([fantom, fantomTestnet, optimism, optimismGoerli], [publicProvider()])
...
export default function PoolCard({ pool }) {
  ...
  async function BuyPresale(){
    newLaunchPool.increaseAllowance(amountToBuy.toString()).then(res=>{
      toast.success("Allowance Success")
      toast.loading("Buying Presale Token")
      newLaunchPool.buyTokens(amountToBuy.toString()).then(res=>{
  ...
  return (
    <div key={pool[0].id} className="pool-container">
      <div className="pool-box">
        ...
        <input type="text" value={amountToBuy} onChange={(e)=>{
